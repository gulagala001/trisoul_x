...
import { windowsBridgeBuild } from '../src/computer-use/windows-build.mjs';
...
try {
  ...
  const args = ['publish', fileURLToPath(new URL('../native/computer-use/windows/TrisoulBrowserBridge.csproj', import.meta.url)), '-c', 'Release', '-r', info.rid, '--self-contained', 'true', '-p:PublishSingleFile=true', '-p:TrisoulBuild=' + info.build, '-p:BaseIntermediateOutputPath=' + intermediate + '/', '-o', output, '--nologo'];
