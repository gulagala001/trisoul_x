...
try{
  ...
  const created=await rpc('session/create',{workspaceId:registered.workspace.workspaceId,agentPreset:'trisoul-x'}),sessionId=created.sessionId;report.sessionId=sessionId;report.origin=origin;
  ...
  const state=await until(async()=>{
    ...
    const r=await fetch(origin+'/trisoul-x/api/state?session='+sessionId,{headers:{cookie}}),s=await r.json();
  ...
  if(connection&&child.exitCode===null)try{
    ...
    const state=await(await fetch(origin+'/trisoul-x/api/state?session='+sessionId,{headers:{cookie},signal:AbortSignal.timeout(5000)})).json();report.metrics=state.metrics;
