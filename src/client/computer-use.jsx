...
export function ComputerPane({sessionId,useTabInfo,inputActions,conversation}){
  ...
  useEffect(()=>{setNavigation(null);},[state?.target?.id]);
  ...
  return <div className="tx-cu-pane"><header>
    ...
    {!state?.target&&<div className="tx-cu-empty"><ScreenIcon/><h3>让助手操作应用和网页</h3><p>在对话中用 @ 选择应用或网页，<br/>也可以打开浏览器开始工作。</p></div>}
    <BrowserControls ref={controls} sessionId={sessionId} state={state} visible={tab.visible} navigation={navigation} api={api} onState={setState} onError={setError}/>
    {state?.target&&state.target.kind!=='tab'&&<p className="tx-cu-target"><ComputerIcon/>{state.target.name??'当前应用'}</p>}
    {(error||state?.lastError)&&<p className="tx-cu-error" role="alert">{error||state.lastError.message}</p>}
    {state?.target?.kind==='tab'?<BrowserPreview key={state.target.id} sessionId={sessionId} tabId={state.target.id} visible={tab.visible} state={state} api={api} url={url} onState={setState} onError={setError} onNavigation={setNavigation} onFrame={setFrame} onBrowserShortcut={action=>controls.current?.shortcut(action)}/>:state?.target?.kind==='app'?<NativePreview key={state.target.viewId} sessionId={sessionId} targetId={state.target.viewId} visible={tab.visible} state={state} url={url} onError={setError}/>:null}
...
function SavedImage({attachment,loadImage}){
  const[src,setSrc]=useState('');useEffect(()=>{let active=true;if(loadImage)void loadImage(attachment).then(value=>{if(active)setSrc(value);}).catch(()=>{});return()=>{active=false;};},[attachment,loadImage]);
  return src?<a href={src} target="_blank" rel="noreferrer"><img className="tx-cu-card-image" src={src} alt="Computer Use 截图"/></a>:null;
}
function ComputerCard({block,loadImage,toolName,openFile}){
  ...
  return <div className="tx-cu-card" data-state={!settled?'running':stopped?'stopped':failure?'error':'idle'}>
    <button type="button" className="tx-cu-card-heading" aria-expanded={open} aria-controls={bodyId} onClick={()=>setOpen(value=>!value)}>
      <span className="tx-cu-card-chevron"><ComputerIcon name="chevron" size={12}/></span><ScreenIcon/>
      ...
      <small className={failure&&!stopped?'tx-cu-error':''}>{status}</small>
    </button>
    {open&&<div className="tx-cu-card-body" id={bodyId}>
      {images.map((c,i)=><SavedImage key={i} attachment={c.attachment} loadImage={loadImage}/>)}
...
export function applyComputerUseClient(ctx, { openPanel, renderPane, integrated = false } = {}){
  ...
  function GroupedComputerCard(props){return <Group {...props}><ComputerCard {...props}/></Group>;}
