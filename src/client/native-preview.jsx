...
export function NativePreview({sessionId,targetId,targetKind='app',stacked=false,visible,state,url,onError}){
  ...
  return <div className="tx-cu-live tx-cu-native-preview" aria-label={targetKind==='tab'?'网页悬浮实时画面':'应用实时画面'}>
    ...
    <div className="tx-cu-live-surface">
      <div className="tx-cu-observed-image">{frame?<img src={'data:'+frame.mediaType+';base64,'+frame.data} alt={targetKind==='app'?'当前应用窗口的实时画面':'当前网页的实时画面'} draggable={false} onLoad={event=>{if(event.currentTarget.src==='data:'+frame.mediaType+';base64,'+frame.data)setDisplayed(frame);}}/>:<div className="tx-cu-live-placeholder">{connection==='disabled'?label:'正在获取应用窗口…'}</div>}
