...
在 DSH 原有鲸鱼轮廓上，加入冰蓝渐变、玻璃高光和蓝紫光轨。静止时微光，悬停时轻轻摆尾；当前会话运行时光点绕行，结束后回到静态。侧栏、欢迎页、网页图标和 Chrome 扩展使用同一套图形。

![鲸鱼图标实际动效：静态微光、悬停摆尾、运行光轨](docs/images/whale-motion.gif)

*由程序实际使用的 SVG/CSS 录制。应用中的动效遵循系统“减少动态效果”设置，支持浅色与深色主题。*
