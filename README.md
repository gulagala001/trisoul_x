...
- **自动记忆整理**：消化结果的 overlap/conflict 信号或已有记忆的实际变更触发整理，沿用原整理提示词；按当前选择的记忆范围处理，保留版本、使用记录和用户来源。会话级记忆沿用原版不触发长期记忆整理。最短间隔默认 3 分钟，可在设置调整；监控中单独显示调用和失败。
- **执行监控**：当前会话及子代理或全部会话，查看主执行、子代理、记忆消化、记忆整理、上下文整理的用量、缓存、失败、耗时、上下文组成及记忆动作。
