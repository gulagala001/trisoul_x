...
- **任务与验证**：`todo_write` 合并原 `task_map` 与 `todo`，负责原文摘录、必需锚点、按项编辑、删除和勾选；独立 `verify_link` 负责关联证据、执行测试、撤销和查看。两者共用稳定任务/证据 ID 和同一份记录；编辑任务仍自动取消完成并清除旧证据。以 DSH `todo/write` 事件持久化，原生清单和工作上下文面板共用数据，页面仍在任务下展示对应证据，继续对话仍可查看。
