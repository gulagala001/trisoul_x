...
- **工作上下文**：右侧栏查看任务与验证、约束、状态、笔记、任务记忆文档、探针补记与工作纪要，也可手动整理较早上下文。
...
- **执行监控**：组件调用轨迹、上下文演变、缓存读取、耗时与失败、记忆/状态/压缩/探针动作。输入区显示当前上下文和运行统计。上下文历史从升级后的新请求开始记录。
- **设置**：后台模型可统一或分别配置；恢复原三档压缩频率和自定义值，以及记忆消化/补注/轮巡、状态、探针、旧用户消息退役和可选上限。保存后下一步/下一批生效；主模型与普通工具仍使用 DSH 自带设置。
...
本次全面恢复共 41 项自动测试通过，包含真实 DSH 的后台设置热更新、状态与记忆分工、探针、快照清理、版本管理和回捞。用户此前审定的提示词删除项与命令安全门排除要求继续有效；完整恢复/适配理由见提示词对照表 P59–P70。
