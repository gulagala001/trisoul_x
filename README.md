...
- **任务与验证**：一个 `todo_write` 合并原 `task_map` / `todo` / `verify_link` 操作，保留原文摘录、必需锚点、稳定任务/证据 ID、按项编辑和勾选。删除统一使用 `op:remove`，`op:view` 在各任务下展示对应证据。编辑任务会自动取消完成并清除旧证据；测试或文字证据可挂接、执行、撤销和查看。以 DSH `todo/write` 事件持久化，原生清单和工作上下文面板共用同一份记录，继续对话仍可查看。
