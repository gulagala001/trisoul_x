...
助手选择目标后，**对话页内自动显示悬浮预览**，多个网页／应用的实时画面以卡片堆叠；可展开选择，也可直接点击露出的卡片进入对应窗口。Mac 应用会显示原窗口；内置网页回到 Oh My DSH 浏览器面板，外置浏览器定位原标签。关闭预览只结束观察，可用输入区的 **悬浮预览** 图标重新打开；停止助手后仍可看图。右上角 **弹出预览** 可选用新版 Chrome 的独立置顶窗口，页内预览本身不依赖此能力。
