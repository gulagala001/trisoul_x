...
需要 Windows 10 2004 或更新版本，支持 x64 与 ARM64 构建。在 **电脑 → 运行环境与权限 → 安装桌面控制** 中编译并安装，再选择已经打开的应用窗口。应用名或窗口不唯一时，选择发现列表中的具体窗口。
...
出现更新提示时点击 **更新桌面控制**；安装损坏时可点击 **修复桌面控制**。更新先校验新程序，切换失败时保留旧安装；之后重新选择应用。当前仅发现已经运行的应用，原生运行时卸载尚未接入；锁屏、提升权限窗口和应用兼容性不能据基础自检结果推定可用。
...
脚本检查插件机制、真实浏览器、Windows 可执行桥接、注册与移除以及 DSH 界面，最后打印 `cu-artifacts/windows-check-...` 结果目录。原生测试会打开两个自有测试窗口，检查控件文字、遮挡下的截图、独立预览、文本输入和停止；请保持桌面解锁，并在这段测试期间暂时不操作鼠标和键盘。发生失败时，保留其中的 `report.json` 和对应日志以便定位；无需提供模型密钥或个人浏览器配置。
...
原生截图与结果写入 `cu-artifacts/windows-native-...`。报告会区分已经执行的键盘／控件检查和仍待验证的物理指针、拖拽、剪贴板及其他场景。
