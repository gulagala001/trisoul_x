...
test('Windows native installation, read-only observation and input operate on real WPF windows', { skip: process.platform !== 'win32' || process.env.TRISOUL_CU_WINDOWS_NATIVE_TEST !== '1' ? 'requires an interactive Windows desktop and TRISOUL_CU_WINDOWS_NATIVE_TEST=1' : false, timeout: 360000 }, async t => {
  ...
  await delay(released.doubleClickTime + 50);
  await native.invoke('controller', binding.id, 'click', [from, { clickCount: 2 }]);
  const doubled = await waitState(state => state.pointerEvents.filter(event => event.kind === 'up').length === 7);
  assert.deepEqual(doubled.pointerEvents.slice(released.pointerEvents.length).filter(event => event.kind === 'down').map(event => event.clickCount), [1, 2], 'Windows and WPF must recognize an actual double click');
  assert.ok(Object.values(doubled.held).every(held => held === false));
  await writeFile(join(artifact, 'pointer-events.json'), JSON.stringify(doubled, null, 2));
  ...
  let tree = (await controlsState()).state;
  await writeFile(join(artifact, 'controls-state.txt'), tree);
  const controlId = name => { const line = tree.split('\n').find(line => line.includes(name)); const id = Number(line?.match(/\[(\d+)\]/)?.[1]); assert.ok(Number.isSafeInteger(id), 'missing observed control: ' + name); return id; };
  await native.invoke('controls', controls.id, 'performSecondaryAction', [controlId('复选操作'), 'toggle']);
  await waitState(state => state.controls.toggled === true);
  await native.invoke('controls', controls.id, 'performSecondaryAction', [controlId('展开操作'), 'expand']);
  await waitState(state => state.controls.expanded === true);
  tree = (await controlsState()).state;
  await native.invoke('controls', controls.id, 'performSecondaryAction', [controlId('展开操作'), 'collapse']);
  const collapsed = await waitState(state => state.controls.expanded === false && state.controls.viewport > 0);
  tree = (await controlsState()).state;
  await native.invoke('controls', controls.id, 'scroll', [controlId('滚动区域'), 'down', 0.5]);
  const scrolled = await waitState(state => state.controls.scrollOffset > 0);
  assert.ok(Math.abs(scrolled.controls.scrollOffset - collapsed.controls.viewport / 2) <= 1, 'half-page scroll moves the actual viewport by half its height');
  tree = (await controlsState()).state;
  await native.invoke('controls', controls.id, 'scroll', [controlId('滚动区域'), 'up', 0.5]);
  const restored = await waitState(state => state.controls.scrollOffset === 0);
  await writeFile(join(artifact, 'controls-result.json'), JSON.stringify({ scrolled: scrolled.controls, restored: restored.controls }, null, 2));
  await native.release('controls');
