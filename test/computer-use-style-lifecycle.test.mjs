...
test('removing Chrome preserves its connection and registration until temporary styles are restored',async t=>{
  const root=await mkdtemp(join(tmpdir(),'trisoul-cu-style-remove-')),manager=new ComputerUseManager(root,{native:{binary:join(root,'missing')}});
  ...
  fail=false;await manager.removeExtension();assert.equal(view.stylePreview,null);assert.equal(closed,1);assert.equal(removed,1);
});
