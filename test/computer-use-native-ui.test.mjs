...
test('DSH native UI: live window, stable stream between turns, cursor and stopped observation',{skip:process.platform!=='darwin'||!process.env.TRISOUL_CU_NATIVE_SOCKET,timeout:60000},async t=>{
  ...
  const originalCard=floating.locator('.tx-cu-preview-card').filter({has:pip.getByRole('button',{name:'放大预览：Oh My DSH UI Fixture',exact:true})});
  const secondCard=floating.locator('.tx-cu-preview-card').filter({has:pip.getByRole('button',{name:'放大预览：Oh My DSH Keyboard Fixture',exact:true})});
